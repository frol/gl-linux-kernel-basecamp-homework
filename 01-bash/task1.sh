...
output "Date: $(date)"
output '---- Hardware ----'
output "CPU: "'"'"$(print_dmidecode 'processor-version')"'"'
output "RAM: $(( $(cat /proc/meminfo | grep MemTotal | awk '{print $2}') / 1024)) MB"
output "Motherboard: "'"'"$(print_dmidecode 'baseboard-manufacturer')"'"'", "'"'"$(print_dmidecode 'baseboard-product-name')"'"'""
output "System Serial Number: $(print_dmidecode 'system-serial-number')"
output '---- System ----'
output "OS Distribution: "'"'"$(. /etc/os-release && echo "$PRETTY_NAME")"'"'
output "Kernel version: $(uname -r)"
output "Installation date: $(ok_or_unknown "$(LANG=C dumpe2fs "$(LANG=C mount | grep 'on \/ ' | awk '{print $1}')" 2>/dev/null | grep 'Filesystem created' | sed 's/^[^:]*: *//')")"
output "Hostname: $(hostname)"
output "Uptime: $(LANG=C uptime | awk '{print $3}' | sed 's/,$//')"
output "Processes running: $(ps -axo pid= | wc -l)"
output "User logged in: $(users | tr ' ' '\n' | wc -l)"
output '---- Network ----'
